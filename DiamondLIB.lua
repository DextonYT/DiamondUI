...
local DiamondFolder = "Diamond"
local ConfigurationFolder = DiamondFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local DiamondQuality = {}

local DiamondLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across Diamond/Arrayfield
...
local Diamond = game:GetObjects("rbxassetid://13853811008")[1]
...
if game["Run Service"]:IsStudio() then
	function gethui() return Diamond end local http_request = nil local syn = {protect_gui = false,request = false,}local http = nil function writefile(tt,t,ttt)end function isfolder(t)end function makefolder(t)end function isfile(r)end function readfile(t)end
end

pcall(function()
	_G.LastRayField.Name = 'Old DiamondUI'
...
local ParentObject = function(Gui)
	...
	_G.LastRayField = Diamond
end
ParentObject(Diamond)
...
local BarType = 'Side'
local HoverTime = 0.3
local Notifications = Diamond.Notifications

local SelectedTheme = DiamondLibrary.Theme.Default
...
function ChangeTheme(ThemeName)
	...
	Diamond.Main.BackgroundColor3 = SelectedTheme.Background
	Diamond.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Diamond.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Diamond.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Diamond.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Diamond.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Diamond.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
		...
		if ArrayFieldLibrary.Flags[FlagName] then
			spawn(function() 
				if DiamondLibrary.Flags[FlagName].Type == "ColorPicker" then
					...
					if DiamondLibrary.Flags[FlagName].CurrentValue or DiamondLibrary.Flags[FlagName].CurrentKeybind or DiamondLibrary.Flags[FlagName].CurrentOption or DiamondLibrary.Flags[FlagName].Color ~= FlagValue then DiamondLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			DiamondLibrary:Notify({Title = "Flag Error", Content = "Diamond was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
function DiamondLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Diamond | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if Diamond.Name == "ArrayField" then
...
function Hide()
	...
	Debounce = true
	DiamondLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function DiamondLibrary:CreateWindow(Settings)
	Diamond.Enabled = false
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Diamond Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Dexton"
	if Settings.LoadingTitle ~= "Diamond Interface Suite" then
		LoadingFrame.Version.Text = "Diamond UI"
	end
	...
	pcall(function()
		...
		if not isfolder(DiamondLibrary.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(DiamondLibrary.."/Discord Invites") then
			makefolder(DiamondFolder.."/Discord Invites")
		end
		if not isfile(DiamondFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(DiamondFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Diamond RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(DiamondFolder.."/Key System") then
			makefolder(DiamondFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Diamond | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(DiamondFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(DiamondFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function(EnterPressed)
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(DiamondFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						DiamondLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
	...
	function Window:CreateTab(Name,Image)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Diamond | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateSection(SectionName,Display)
			...
			if Display then
				...
				SectionValue.Holder.Parent = Diamond.Holding
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function(enter)
				...
				if not Success then
					...
					print("Diamond | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			local function AddOption(Option,Selecteds)
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Diamond | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
					end
			...
			local function AddOptions(Options,Selected)
				...
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
						DiamondLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
					end
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then  --Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Diamond | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					DiamondLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Diamond | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					DiamondLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					DiamondLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			local function UpdateSlider(X)
				...
				if SliderSettings.CurrentValue ~= NewValue then
					...
					if not Success then
						...
						print("Diamond | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Slider.Title.Text = SliderSettings.Name
						TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					DiamondLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
			...
			for name,info in pairs(PromptSettings.Actions) do
				...
				Button.Interact.MouseButton1Up:Connect(function()
					if not clicked then
						...
						if not Success then
							ClosePrompt()
							print("Diamond | "..info.Name.." Callback Error " ..tostring(Response))
						else
							ClosePrompt()
						end
					end
...
function DiamondLibrary:Destroy()
	Diamond:Destroy()
end
...
function DiamondLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				DiamondLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end
task.delay(9, DiamondLibrary.LoadConfiguration, DiamondLibrary)
return DiamondLibrary
